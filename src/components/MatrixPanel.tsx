...
export function MatrixPanel({ conversation }: { conversation: Conversation }): JSX.Element | null {
  ...
  return (
    <div className="border-l border-neutral-700 bg-neutral-900 px-3 py-2">
      ...
      <table className="text-[10px]">
        <thead>
          <tr>
            <th />
            {personas.map((p) => (
              <th key={p.id} className="px-1 text-center font-normal text-neutral-300">
                {short(p.name)}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {personas.map((observer) => (
            <tr key={observer.id}>
              <td className="pr-1 text-right text-neutral-300">{short(observer.name)}</td>
              {personas.map((source) => (
