...
function renderBubbleBody(message: Message, excluded: boolean): JSX.Element {
  ...
  if (body.kind === "html") {
    // Markdown HTML is generated by our own escaping renderer in
    // rendering/markdown.ts — input is escaped before span markers
    // get re-introduced, so dangerouslySetInnerHTML is safe here.
    return (
      <div
        className={`markdown-body text-sm leading-relaxed ${tone}`}
