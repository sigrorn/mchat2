...
export async function importSnapshot(snapshot: SnapshotEnvelope): Promise<ImportResult> {
  // #269: every DB write below runs inside one transaction so a
  // partial failure (provider rejection, schema constraint, OS
  // interrupt) rolls back to pre-import state instead of leaving a
  // half-formed conversation in the sidebar. Keychain checks +
  // openai_compat config writes + the apertus auto-migrator stay
  // outside the transaction (the first because keychain is non-DB,
  // the others because they touch global settings/keychain that the
  // import-time conversation doesn't depend on for visibility).
  const {
    conv: updatedConv,
    ...
    return { conv: updatedConv, legacyApertusProductIdFromSnapshot };
  });
  ...
  await migrateApertusInConversation(updatedConv.id);
